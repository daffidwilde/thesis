...
def get_contribution_variation(df):
    ...
    cont_var = pd.concat([contributions, variations[components]], axis=1) \
                 .rename({0: 'contribution', 1: 'variation'}, axis=1)

    new_idx = np.random.permutation(cont_var.index)
    cont_var = cont_var.reindex(new_idx)
...
def make_legend(ax):
    ...
    """
    diab_outer = ax.scatter([], [], marker='o', s=100, alpha=0.75,
                            edgecolor='None', facecolor=green)
    diab_inner = ax.scatter([], [], marker='.', s=10, color='w')
    nondiab_outer = ax.scatter([], [], marker='o', s=100, alpha=0.75,
                               edgecolor='None', facecolor=blue)
    nondiab_inner = ax.scatter([], [], marker='.', s=10, edgecolor='None',
                               facecolor=blue)

    patches = ((nondiab_outer, nondiab_inner), (diab_outer, diab_inner))
    legend = ax.legend(patches, (('non-diabetic'), ('diabetic')), loc='best',
                       fontsize=20, markerscale=2.5)
...
def make_size_legend(ax):
    ...
    for i, size in enumerate(sizes):
        ax.scatter([], [], marker='.', s=size*1e3, alpha=0.75,
...
def plot_cost_bubble(df):
    ...
    args = zip(get_contribution_variation(nondiabetic).iterrows(),
    ...
    for nondiab_row, diab_row in args:
        ...
        for i, (component, point) in enumerate([nondiab_row, diab_row]):
            ...
            ax.scatter(component, contribution, s=1e3*variation, marker='.',
                       ...
                       edgecolor='None', facecolor=face)
            ax.scatter(component, contribution, s=100, marker='.',
                       zorder=2.5-(0.01*variation), color=centre)
