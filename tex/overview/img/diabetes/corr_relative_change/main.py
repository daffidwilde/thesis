...
def plot_corr_relative_change(df):
    """
    Heatmap showing the relative change in the pairwise correlation
    coefficients of our cost components and some other clinical variables at the
    spell level, between diabetic patients and the general dataset.
    """
    ...
    relative_change = 100 * (corrs[0] - corrs[1]) / corrs[1]
    relative_change.replace([-np.infty, np.infty], np.nan, inplace=True)
