...
def plot_corr_heatmap(df):
    ...
    and some other clinical variables at the spell level, for diabetic patients.
    """
    ...
    summed_costs = diabetic.groupby('SPELL_ID')[costs].sum()
    lengths_of_stay = diabetic.groupby('SPELL_ID')['TRUE_LOS'].mean()
    max_diagnosis_nums = diabetic.groupby('SPELL_ID')['DIAG_NO'].max()
    ...
    sbn.heatmap(correlation, square=True, cmap='viridis', vmin=-1, vmax=1,
                lw=0.01, annot=True, annot_kws={'fontsize': 6}, ax=ax)
