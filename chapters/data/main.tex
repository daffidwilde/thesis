...
\begin{itemize}
    ...
    \item and (separately) the number of spells associated with
any given patient.
\end{itemize}

\subsection{Cleaning the data}\label{subsec:formatting}

As with any data analysis, a substantial amount of preprocessing
...
to visit a hospital would be reaching their peak healthiness.
...
Looking at these key attributes, it would appear things are as expected:
...
section gave a good base for understanding the scope of the data. The
next step is to investigate how these key attributes interact with one
...
correlation here only adds to the evidence that the patients in the data
...
At the other end of Figure~\ref{fig:corr_heatmap}, ward and overhead costs have some of the
...
Other than the few attributes at the top, this heat map shows that
...
Here, each of the chosen attributes
...
of the dataset. Following advice from the subject matter expert, the diabetic population was used. The
...
So what was there to be gained by looking at the diabetic population? The lack of
