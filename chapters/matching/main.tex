...
\begin{definition}\label{def:sm_game}
    ...
    \emph{game} of size \(N\), denoted \(\left(S, R\right)\), and is used to
    model instances of SM.
\end{definition}

\begin{definition}\label{def:sm_matching}
    Consider a game \(\left(S, R\right)\). A \emph{matching} \(M\) is any
    bijection between \(S\) and \(R\). If a pair \(\left(s, r\right) \in S
    \times R\) are matched in \(M\), then that relationship is denoted \(M(s) =
    r\) and, equivalently, \(M^{-1}(r) = s\).

    A matching is considered valid only if every player in \(\left(S, R\right)\)
    is matched to another player uniquely.
\end{definition}

\begin{definition}\label{def:sm_preference}
    Let \(\left(S, R\right)\) be an instance of SM, and consider \(s \in S\) and
...
\begin{definition}\label{def:sm_blocking}
    Let \(\left(S, R\right)\) be an instance of SM and let \(M\) be a matching
    of \(\left(S, R\right)\). A pair \(\left(s, r\right) \in S \times R\) is
    said to \emph{block} \(M\) if:
