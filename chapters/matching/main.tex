...
\begin{figure}[htbp]
    \centering
    \input{chapters/matching/paper/docs/tex/hr_matching}
...
\begin{figure}[htbp]
    \centering
    \input{chapters/matching/paper/docs/tex/hr_invalid}
...
\begin{figure}[htbp]
    \centering
    \input{chapters/matching/paper/docs/tex/hr_unstable}
...
\begin{figure}[htbp]
    \centering
    \input{chapters/matching/paper/docs/tex/hr_stable}
    \caption{%
...
at hospital \(C\), falsely assuming that this will guarantee them a place at
\(C\). The HR game stipulates that a stable matching does not require all
residents to be matched, and so, a situation could arise where \(A\) will not be
assigned to a hospital. For instance, if \(C\) swapped \(A\) and \(S\) in its
preference list then \((S, C)\) would form a blocking pair under this matching.
The only resolution that gives a stable matching then is to leave \(A\) without
a match, and for \(M(C) = \left\{S, D\right\}\).
