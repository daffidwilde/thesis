...
\begin{figure}
    ...
    \begin{subfigure}{\imgwidth}
        ...
        \caption{}\label{fig:kmeans_inertia_fitness}
    \end{subfigure}

    \begin{subfigure}{\imgwidth}
        \includegraphics[width=\linewidth]{kmeans_inertia_nrows.pdf}%
        \caption{}\label{fig:kmeans_inertia_nrows}
    \end{subfigure}
    \caption{%
...
\subsection{The silhouette coefficient}
...
the EA (see Figure~\ref{fig:kmeans_inertia_fitness}) --- rendering the exercise
pointless. However, its effect integrates well with the silhouette coefficient:
\begin{itemize}
    \item The optimal score is the same (\(1 - 0 = 1\)) as the silhouette
        fitness function while the worst score is similar (\(-1-1=-2\)).
    \item A score of zero still indicates there is little to be gained in that
        ...
        cloud.
    \item Negative scores indicate a dataset with either a low-to-negative
        silhouette coefficient and high correlation (i.e.\ high inertia but
        correlated) or negative silhouette (i.e.\ the clustering is
        inappropriate) and arbitrary correlation.
\end{itemize}
...
in this metric on the two algorithms. In terms of EDO, this means using \(f =
g_A - g_B\), \(f = g_B - g_A\) or \(f = \left| g_B - g_A \right|\) as the
fitness function. Doing so can highlight pitfalls, edge cases or fundamental
conditions for the method(s). Overall, this process affords a deeper level of
learning about the method of interest beyond the traditional empirical method of
literature comparison on a particular example.
...
Clustering of Applications with Noise (DBSCAN). To begin with, the objective is
to find datasets for which \(k\)-means outperforms its alternative, DBSCAN.\
There is no concept of inertia in DBSCAN as it is based on density (as opposed
to raw distance) and identifies outliers~\cite{Ester1996}. As such, a valid
metric must be chosen. Again, the silhouette coefficient is one such metric. The
standard silhouette function is used here over that defined
...
X\) be the clustering by DBSCAN. Then the fitness function is defined to be:
\begin{equation}
    f(X) =
        \begin{cases}
...
in the range \([-2, 2]\) where \(2\) is the best, i.e.\ \(S_D(X) = -1\) and
\(S_k(X) = 1\). Likewise, -2 is the worst score. Finally, any individual that is
not clustered into at least two parts by DBSCAN is penalised heavily under this
fitness function when, in fact, that clustering may be of high quality. As such,
this fitness function may require more nuanced adjustment.
...
found. The \emph{concave hull} of a cluster is taken to be the \(\alpha\)-shape of
