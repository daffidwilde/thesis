...
Choosing an optimal set of parameters for any given run of EDO is not an objective exercise
...
The remainder of the process is a cycle of studying the generated
...
outperforms \(k\)-means with some altered silhouette coefficient. Given the
success of the \(k\)-means-preferable fitness function, a sensible starting
point for the fitness function in this case would be to alter \(f\) slightly. To
be specific, the objective is the reverse of \(f\) and so the fitness function
should be \(-f\). However, the same penalty of \(-\infty\) is required for the
case set out in~\eqref{eq:kmeans_preferable}, where DBSCAN produces only one
cluster. This altered fitness function is referred to as the
\emph{DBSCAN-preferable} fitness function.
...
This iterative approach in adjusting the fitness function also suggests that
obtaining an optimal parameter set is less important than in some applications
of EAs. For instance, the only probability distribution family used in the case
study was the uniform distribution. However, in the most successful cases, the
distributions of the individual datasets were not at all uniform; this is a
direct result of the fitness function being so freely modifiable. By adjusting
the fitness function appropriately, the EA was able to avoid structural
properties in the data it was producing, thus transcending the other parameters.
...
datasets exist is not well-documented in literature, and this thesis established
Evolutionary Dataset Optimisation as a starting point for further works to come.
