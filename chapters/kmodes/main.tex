...
This chapter focuses on \(k\)-modes clustering, an extension to \(k\)-means that
permits the sensible clustering of categorical (i.e.\ ordinal, nominal or
...
this chapter is how the performance of the \(k\)-modes algorithm is affected by
its initial solution. The initialisation method described herein extends the
method presented by Huang~\cite{Huang1998} by using results from game theory
to ensure mathematical fairness, and to lever the full learning opportunities
presented by the data being clustered.
...
initialisation also forms the basis of a number of other initialisations where a
notion of density is central. This chapter builds up to an analysis of these
methods against the proposed, and it is demonstrated that the proposed method is
able to outperform them both. The aim of this analysis is two-fold: first, to
highlight the merits of the method in a familiar setting by clustering
well-known benchmark datasets; and second, to provide a deeper insight into how
the methods perform against one another by generating artificial datasets using
the Evolutionary Dataset Optimisation method introduced in
Chapter~\ref{chp:edo}.
...
from these use cases to use Huang's method over the one proposed in this chapter.
...
prior knowledge to be included in an initial clustering by using some ad hoc
