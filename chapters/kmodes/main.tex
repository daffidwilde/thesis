...
than their current matches is called \emph{solving} the game.~\cite{Gale1962}
formalised this problem and provided an algorithm that guarantees to find an
stable, party-optimal matching to any instance of SM. This algorithm is given in
Appendix~\ref{app:matching}.
...
A popular generalisation of SM is the hospital-resident assignment problem (HR),
a matching game that addresses the practical problem from which it gets its
name: assigning medical students to hospital placements. A variant of this game
is still used by the National Resident Matching Program (NRMP) in the United
States. A thorough description of HR, and the algorithms to solve instances of
it, is provided in Appendix~\ref{app:matching}. However, a brief overview is
provided here.
...
H\) has a strict upper limit on the number of residents they may be matched to
at once, referred to as their capacity, and denoted \(c_h \in \mathbb N\). A
general assumption, although not necessary, is that there is sufficient capacity
across the hospitals for all of the residents to be assigned, i.e. that
\(\sum_{h \in H} c_h \ge \lvert R \rvert\). This construction of residents,
hospitals, preference lists and capacities form a \emph{game}, denoted by \((R,
H)\).
...
stable matching to an instance of HR is called \emph{solving} the
game.~\cite{Gale1962} formalised this game and presented an algorithm that
guaranteed such a solution to any instance of HR. Subsequent
studies~\cite{Dubins1981,Roth1984} have improved on the original algorithm by
leveraging structures within the game, such as the strict nature of the
preference lists.
