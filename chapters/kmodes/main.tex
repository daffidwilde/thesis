...
in~\eqref{eq:dissim}. Further to its alternative, density-based approach, the
method (which is described in Algorithm~\ref{alg:cao}) is deterministic.
Therefore, when using \(k\)-modes with Cao's initialisation, only one run of the
algorithm is required to identify a clustering, making it an attractive option
if computational time is critical.
...
\begin{definition}\label{def:density}
    ...
    Observe that \(\text{Dens}_j \left(X^{(i)}\right)\) is the relative
    frequency of \(x_j^{(i)}\) in \(\mathcal X\), and:
    \begin{equation}
...
potential modes and some subset of the data. The particular constraints of this
case --- where the \(k\) potential modes must be allocated to a nearby unique
data point --- mirror those of the hospital-resident assignment problem (HR)
discussed in Chapter~\ref{chp:matching}. A contextual summary of the HR game
components and those of the proposed initialisation is given in
Table~\ref{tab:components}. Algorithm~\ref{alg:proposed_method} provides a
