...
\(k\)-modes and \(k\)-means algorithms. A full statement of the algorithm
(adapted from~\cite{Huang1997a}) is given in Algorithm~\ref{alg:kprototypes}. In
essence, the \(k\)-prototypes algorithm separates the given dataset into its
numeric and categorical attributes before applying \(k\)-means and \(k\)-modes
...
according to a single weight, \(\gamma \in \mathbb R\). Using some of the same
notation as in Chapter~\ref{chp:kmodes}, let the attribute space be written as:

\begin{equation}
    \mathcal A = \Pi_{j=1}^{p} A^{(n)}_j \times \Pi_{j=p+1}^{m} A^{(c)}_j
\end{equation}

\noindent where \(A^{(n)}\) and \(A^{(c)}\) denote numeric and categorical
attributes, respectively. Then, the dissimilarity between two points, \(X, Y \in
